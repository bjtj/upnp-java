...
public class UPnPControlPoint {
    ...
    private void dispatch(SSDPHeader header) {
        ...
        long timeout = Math.max(1800, header.getMaxAge()) * 1000;
        ...
        try {
            ...
            addDevice(location, builder.build(location), timeout);
    ...
    public void addDevice(URL baseUrl, UPnPDevice device, long timeout) {
        sessions.put(device.getUdn(), UPnPDeviceSession.withDeviceAndTimeout(baseUrl, device, timeout));
        for (OnDeviceListener listener : deviceListenerList) {
